...
(function (window) {
    mini.namespace("Group.Accounts");
    var self = Group.Accounts;
    self.prifix = "AC_Group_Accounts_";
    ...
    function directRemove(id) {
        if (id) {
            ...
            $.post(bootPATH + "../AC/Group/RemoveGroupAccounts", { id: id }, function (result) {
    ...
    function onSelectOk(accountDic) {
        ...
        $.post(bootPATH + "../AC/Group/AddGroupAccounts",
            data,
            function (result) {
                helper.response(result, function () {
                    grid.reload();
    ...
    function ondrawcell(e) {
        ...
        if (columnName && columnName == "action") {
            var record = e.record;
            e.cellHtml = '<a title="删除" href="javascript:Group.Accounts.remove(\'' + record.Id + '\')"><img alt="删除" border="0" src="' + bootPATH + '../Scripts/miniui/themes/icons/remove.gif" /></a>';
        }
    };
})(window);