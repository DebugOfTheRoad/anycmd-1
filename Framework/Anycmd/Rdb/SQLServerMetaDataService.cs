...
@" DROP TABLE #T";
        ...
        public IList<RDatabase> GetDatabases()
        {
            ...
            var sql = "select * from [RDatabase] order by CatalogName";
            using (var conn = new SqlConnection(this.BootConnString))
            {
                var cmd = conn.CreateCommand();
                cmd.CommandText = sql;
        ...
        {
            var sql = "update [RDatabase] set DataSource=@DataSource,Description=@Description where Id=@Id";
            using (var conn = new SqlConnection(this.BootConnString))
            {
                var cmd = conn.CreateCommand();
                cmd.CommandText = sql;
        ...
        {
            var addProcName = "sys.sp_addextendedproperty";
            var updateProcName = "sys.sp_updateextendedproperty";
            var dropProcName = "sys.sp_dropextendedproperty";
            var propertyName = "MS_Description";
            switch (metaDataType)
            {
                ...
                case RDbMetaDataType.Table:
                    #region Table
                    DbTable table;
                    if (!db.AppHost.DbTables.TryGetDbTable(db, id, out table))
                    {
                ...
                case RDbMetaDataType.View:
                    #region View
                    DbView view;
                    if (!db.AppHost.DbViews.TryGetDbView(db, id, out view))
                    {
                ...
                case RDbMetaDataType.TableColumn:
                    ...
                    if (!db.AppHost.DbTableColumns.TryGetDbTableColumn(db, id, out tableColumn))
                    {
                ...
                case RDbMetaDataType.ViewColumn:
                    ...
                    if (!db.AppHost.DbViewColumns.TryGetDbViewColumn(db, id, out viewColumn))
                    {
