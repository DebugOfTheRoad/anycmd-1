...
namespace Anycmd.AC.Infra
{
    ...
    /// <summary>
    /// 组织结构。组织结构是对资源的单元划分。组织结构属于一种分类法，它与普通的分类法的唯一差别是组织结构具有偏移量。
    /// <example>
    /// 给定一个资源集合Set{0,100}。
    /// 普通的分类法所做的分类类似这样：
    /// {
    ...
    /// </example>
    /// <remarks>
    /// Anycmd对组织结构的使用几乎遍布所有资源，组织结构是资源的附加属性，而资源的固有属性只是没有偏移的分类。Anycmd希望在需要的
    /// 时候能够被允许侵入应用系统的数据库设计以保存资源管理员添加的组织属性（这个组织属性可能不止一种，但不管资源管理员在管理资源集
    /// 的时候添加了多少组织属性Anycmd都只会使用你对应的数据库表的一个字段存储）。
    /// 之所以要侵入应用数据库表是为了根据组织结构对资源集的筛选能够在数据库进程完成而不必加载进应用系统的内存再筛选。
    /// 框架使用那一个字段比如varchar(100)，框架把它的长度再做单元划分，如果一个单元定长为10就可以存储资源管理员添加的10种组织结构。
    /// 根据组织结构查询的时候应用系统发出的sql中会截取这个字段的正确的单元。
    /// Anycmd.AC是个权限框架，也支持实体级的控制。管理资源的时候尽量不推荐使用实体级的控制，首先评估实体属性（在实体属性上可以对资源集进行分类划分），
    /// 如果在实体上的那些固有属性上难以制定出满足安全需求的策略的话再考虑是不是需要组织结构，尽量使用附加的组织属性去对资源划分单元，按照单元进行控制
    ...
    public interface IOrganization
    {
        ...
        /// 编码。
        /// <remarks>
        /// 设计通过以父级组织结构编码为前缀来表现层级关系而不记录ParentID也不记录层级的Level。
        /// </remarks>
        /// </summary>
        string Code { get; }
