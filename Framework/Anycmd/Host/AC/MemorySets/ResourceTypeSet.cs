...
namespace Anycmd.Host.AC.MemorySets
{
    ...
    /// 资源上下文
    /// </summary>
    public sealed class ResourceTypeSet : IResourceSet
    {
        ...
        public ResourceTypeSet(AppHost host)
        {
            if (host == null)
            {
            ...
            IHandler<ResourceTypeRemovedEvent>
        {
            ...
            private void Handle(IResourceCreateInput input, bool isCommand)
            {
                ...
                lock (this)
                {
                    ...
                    if (host.ResourceSet.TryGetResource(input.Id.Value, out resource))
                    {
                    ...
                    if (host.ResourceSet.TryGetResource(appSystem, input.Code, out resource))
                    {
            ...
            private void Handle(IResourceUpdateInput input, bool isCommand)
            {
                ...
                if (!host.ResourceSet.TryGetResource(input.Id, out bkState))
                {
                ...
                lock (bkState)
                {
                    ...
                    if (!host.ResourceSet.TryGetResource(input.Id, out oldState))
                    {
                        throw new NotExistException();
                    }
                    ResourceTypeState resource;
                    if (host.ResourceSet.TryGetResource(appSystem, input.Code, out resource) && resource.Id != input.Id)
                    {
                        throw new ValidationException("重复的资源编码" + input.Code);
                    }
            ...
            private void Handle(Guid resourceTypeID, bool isCommand)
            {
                ...
                var resourceRepository = host.GetRequiredService<IRepository<ResourceType>>();
                ResourceTypeState bkState;
                if (!host.ResourceSet.TryGetResource(resourceTypeID, out bkState))
                {
                    return;
                }
                ResourceType entity;
                lock (bkState)
                {
                    ...
                    if (!host.ResourceSet.TryGetResource(resourceTypeID, out state))
                    {
                        return;
                    }
