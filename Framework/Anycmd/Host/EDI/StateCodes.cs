...
namespace Anycmd.Host.EDI
{
    ...
    {
        public static readonly StateCodes Empty = new StateCodes(AppHost.Empty);
        ...
        /// 构造并接入总线
        /// </summary>
        public StateCodes(IAppHost host)
        {
            this.host = host;
            // TODO:总线接入
        }
            ...
            if (!_initialized)
            {
                lock (locker)
                {
                    if (!_initialized)
                    {
                        ...
                        var stateCodeRepository = host.GetRequiredService<IRepository<StateCode>>();
