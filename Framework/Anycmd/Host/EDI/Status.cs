...
namespace Anycmd.Host.EDI {
    ...
    public static class StatusExtension {
        ...
        public static bool TryParse(this int number, out Status value) {
            ...
            return StateCodeDic.TryGetValue(number, out value);
        }
        ...
        private class StateCodeDic {
            ...
            private static readonly Dictionary<int, string> _dicByInt = new Dictionary<int, string>();
            private static readonly Dictionary<int, Status> _dicByNumber = new Dictionary<int, Status>();
            ...
            public string this[Status stateCode] {
                get {
                    return _dicByInt[(int)stateCode];
                }
            }

            public static bool TryGetValue(int number, out Status value) {
                return _dicByNumber.TryGetValue(number, out value);
            }
            ...
            public string this[int stateCode] {
                get {
                    if (!_dicByInt.ContainsKey(stateCode)) {
                        throw new CoreException("意外的状态码:" + stateCode.ToString());
                    }

                    return _dicByInt[stateCode];
                }
            }

            #region private methods
            private void Init() {
                string[] _names = Enum.GetNames(typeof(Status));
                var _values = Enum.GetValues(typeof(Status)) as short[];
                if (_values == null) {
                    throw new CoreException("枚举Status不是int型的");
                }
                for (int i = 0; i < _values.Length; i++) {
                    _dicByInt.Add(_values[i], _names[i]);
                    _dicByNumber.Add(_values[i], (Status)_values[i]);
                }
            }
            #endregion
        }
