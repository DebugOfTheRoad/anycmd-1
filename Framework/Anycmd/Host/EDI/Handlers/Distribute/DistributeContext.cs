...
namespace Anycmd.Host.EDI.Handlers.Distribute {
	...
	public sealed class DistributeContext {
				...
				if (clientAgent == null) {
					...
					switch (this.Command.ClientType) {
						...
						case ClientType.Node:
							NodeDescriptor node;
							if (!NodeHost.Instance.Nodes.TryGetNodeByID(this.Command.ClientID, out node)) {
		...
		public OntologyDescriptor Ontology {
			get {
				if (ontology == null) {
					if (!NodeHost.Instance.Ontologies.TryGetOntology(this.Command.Ontology, out ontology)) {
