...
namespace Anycmd.Host.EDI.MemorySets.Impl
{
    ...
    public sealed class InfoDicSet : IInfoDicSet
    {
        ...
        /// 构造并接入总线
        /// </summary>
        public InfoDicSet(IAppHost host)
        {
            if (host == null)
            {
            ...
            if (!_initialized)
            {
                lock (locker)
                {
                    if (!_initialized)
                    {
                        ...
                        var allInfoDics = host.GetRequiredService<INodeHostBootstrap>().GetInfoDics();
                        foreach (var infoDic in allInfoDics)
                        {
                            ...
                            var infoDicState = InfoDicState.Create(host, infoDic);
                        ...
                        var allDicItems = host.GetRequiredService<INodeHostBootstrap>().GetInfoDicItems();
            ...
            IHandler<InfoDicItemRemovedEvent>
        {
            ...
            private void Handle(IInfoDicCreateInput input, bool isCommand)
            {
                ...
                lock (locker)
                {
                    ...
                    if (host.InfoDics.TryGetInfoDic(input.Id.Value, out infoDic))
                    {
                        throw new ValidationException("给定标识标识的记录已经存在");
                    }
                    if (host.InfoDics.TryGetInfoDic(input.Code, out infoDic) && infoDic.Id != input.Id.Value)
                    {
            ...
            private void Handle(IInfoDicUpdateInput input, bool isCommand)
            {
                ...
                InfoDicState infoDic;
                if (!host.InfoDics.TryGetInfoDic(input.Id, out infoDic))
                {
                    throw new NotExistException();
                }
                if (host.InfoDics.TryGetInfoDic(input.Code, out infoDic) && infoDic.Id != input.Id)
                {
                    throw new ValidationException("重复的编码");
                }
            ...
            private void Handle(Guid infoDicID, bool isCommand)
            {
                ...
                InfoDicState infoDic;
                if (!host.InfoDics.TryGetInfoDic(infoDicID, out infoDic))
                {
                    return;
                }
                var infoDicItems = host.InfoDics.GetInfoDicItems(infoDic);
            ...
            private void Handle(IInfoDicItemCreateInput input, bool isCommand)
            {
                ...
                InfoDicState infoDic;
                if (!host.InfoDics.TryGetInfoDic(input.InfoDicID, out infoDic))
                {
                    throw new ValidationException("意外的信息字典标识");
                }
                InfoDicItemState infoDicItem;
                if (host.InfoDics.TryGetInfoDicItem(input.Id.Value, out infoDicItem))
                {
                    throw new ValidationException("给定的标识标识的记录已经存在");
                }
                if (host.InfoDics.TryGetInfoDicItem(infoDic, input.Code, out infoDicItem))
                {
                    throw new ValidationException("重复的编码");
                }
            ...
            private void Handle(IInfoDicItemUpdateInput input, bool isCommand)
            {
                ...
                InfoDicItemState infoDicItem;
                if (!host.InfoDics.TryGetInfoDicItem(input.Id, out infoDicItem))
                {
                    throw new NotExistException();
                }
                if (host.InfoDics.TryGetInfoDicItem(infoDic, input.Code, out infoDicItem) && infoDicItem.Id != input.Id)
                {
                    throw new ValidationException("重复的编码");
                }
            ...
            private void HandleItem(Guid infoDicItemID, bool isCommand)
            {
                ...
                var infoDicItemRepository = host.GetRequiredService<IRepository<InfoDicItem>>();
                InfoDicItemState infoDicItem;
                if (!host.InfoDics.TryGetInfoDicItem(infoDicItemID, out infoDicItem))
                {
                    return;
                }
                InfoDicState infoDic;
                if (!host.InfoDics.TryGetInfoDic(infoDicItem.InfoDicID, out infoDic))
                {
