...
namespace Anycmd.Host.EDI.MemorySets.Impl
{
    ...
    public sealed class NodeSet : INodeSet
    {
            ...
            if (!initialized)
            {
                lock (locker)
                {
                    if (!initialized)
                    {
                        ...
                        var allNodes = host.GetRequiredService<INodeHostBootstrap>().GetNodes();
                        foreach (var node in allNodes)
                        {
                            ...
                            var nodeState = NodeState.Create(host, node);
                            var descriptor = new NodeDescriptor(host, nodeState);
                            ...
                            if (node.Id.ToString().Equals(host.Config.ThisNodeID, StringComparison.OrdinalIgnoreCase))
                            {
                                _selfNode = descriptor;
                            }
                            if (node.Id.ToString().Equals(host.Config.CenterNodeID, StringComparison.OrdinalIgnoreCase))
                            {
        ...
        private sealed class NodeElementActionSet
        {
            ...
            internal NodeElementActionSet(IAppHost host)
            {
                if (host == null)
                {
                    throw new ArgumentNullException("host");
                    ...
                    lock (this)
                    {
                        if (!initialized)
                        {
                            ...
                            foreach (var item in nodeElementActions)
                            {
                                ...
                                ElementDescriptor element = host.Ontologies.GetElement(item.ElementID);
                                if (!_nodeElementActionDic.ContainsKey(node))
                                {
        ...
        private sealed class NodeCareSet
        {
            ...
            internal NodeCareSet(IAppHost host)
            {
                if (host == null)
                {
                    throw new ArgumentNullException("host");
                ...
                if (!initialized)
                {
                    lock (_locker)
                    {
                        if (!initialized)
                        {
                            ...
                            var nodeOntologyCareStates = host.GetRequiredService<INodeHostBootstrap>().GetNodeOntologyCares().Select(a => NodeOntologyCareState.Create(a));
                            var nodeElementCareStates = host.GetRequiredService<INodeHostBootstrap>().GetNodeElementCares().Select(a => NodeElementCareState.Create(a));
                            ...
                            {
                                foreach (var element in host.Ontologies[ontology.Ontology.Id].Elements.Values)
                                {
                                    foreach (var node in host.Nodes)
                                    {
                                        if (element == null)
            ...
            private void Init()
            {
                if (!initialized)
                {
                    lock (locker)
                    {
                        ...
                        {
                            _dic.Clear();
                            var ontologyOrgs = host.GetRequiredService<INodeHostBootstrap>().GetNodeOntologyOrganizations();
                            foreach (var nodeOntologyOrg in ontologyOrgs)
                            {
                                OrganizationState org;
                                NodeDescriptor node;
                                OntologyDescriptor ontology;
                                host.Nodes.TryGetNodeByID(nodeOntologyOrg.NodeID.ToString(), out node);
                                host.Ontologies.TryGetOntology(nodeOntologyOrg.OntologyID, out ontology);
                                if (host.OrganizationSet.TryGetOrganization(nodeOntologyOrg.OrganizationID, out org))
                                {
                                    if (!_dic.ContainsKey(node))
                                    {
                                    ...
                                    var nodeOntologyOrgState = NodeOntologyOrganizationState.Create(host, nodeOntologyOrg);
